...
bool TextureShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitShader(device, hwnd, L"../DX12/TextureVertexShader.hlsl", L"../DX12/TexturePixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool TextureShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	bool result;
...
void TextureShader::RenderShader(ID3D11DeviceContext* deviceContext, int indexCount, int instanceCount)
{
	// Set the vertex input layout
	deviceContext->IASetInputLayout(layout);

	// Set the vertex and pixel shaders that will be used to render this polygon
