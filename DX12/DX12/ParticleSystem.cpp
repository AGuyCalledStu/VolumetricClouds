...
// TODO - RasterTek Tutorial 39 (Use this as a base to build a voxel shader for clouds)
...
bool ParticleSystem::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* textureFilename)
{
	bool result;
...
bool ParticleSystem::LoadTexture(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WCHAR* filename)
{
	bool result;
	...
	result = m_Texture->Init(device, deviceContext, (char*)filename);
	if (!result)
	{
		return false;
	}
...
bool ParticleSystem::UpdateBuffers(ID3D11DeviceContext* deviceContext)
{
	...
	for (i = 0; i<m_currentParticleCount; i++)
	{
		...
		m_vertices[index].colour = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;
